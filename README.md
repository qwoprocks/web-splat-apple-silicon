...
The original repository can be found [here](https://github.com/KeKsBoTer/web-splat)

This fork rewrites the radix sort for use on Apple Silicon (M-series) machines, as there was an issue with the original implementation that caused the renderer to freeze on these machines.

The underlying issue was the ordering of workgroup dispatch on M-series GPUs, which caused deadlocks in some parts of the radix sort.
